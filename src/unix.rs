...
use rustix::mm::Advice as RAdvice;
...
impl MmapInner {
    ...
    fn new(
        ...
        unsafe {
            match rustix::mm::mmap(
                ...
                ProtFlags::from_bits_truncate(prot as _),
                MapFlags::from_bits_truncate(flags as _),
                BorrowedFd::borrow_raw(file),
                aligned_offset as _,
            ) {
                Ok(ptr) => Ok(MmapInner {
    ...
    fn mprotect(&mut self, prot: libc::c_int) -> io::Result<()> {
        unsafe {
            ...
            let len = len.max(1);
            match rustix::mm::mprotect(ptr, len, MprotectFlags::from_bits_truncate(prot as _)) {
                Ok(_) => Ok(()),
                Err(e) => Err(io::Error::from_raw_os_error(e.raw_os_error())),
            }
        }
    ...
    pub fn advise(&self, advice: Advice, offset: usize, len: usize) -> io::Result<()> {
        ...
        let len = len + alignment;
        unsafe {
            if let Err(e) = rustix::mm::madvise(self.ptr.offset(offset), len, advice.into()) {
    ...
    pub fn lock(&self) -> io::Result<()> {
        unsafe {
            if let Err(e) = rustix::mm::mlock(self.ptr, self.len) {
                Err(io::Error::from_raw_os_error(e.raw_os_error()))
            } else {
                Ok(())
            }
    ...
    pub fn unlock(&self) -> io::Result<()> {
        unsafe {
            if let Err(e) = rustix::mm::munlock(self.ptr, self.len) {
                Err(io::Error::from_raw_os_error(e.raw_os_error()))
            } else {
                Ok(())
            }
...
impl Drop for MmapInner {
    fn drop(&mut self) {
        ...
        unsafe {
            let ptr = self.ptr.offset(-(alignment as isize));
            let _ = rustix::mm::munmap(ptr, len as libc::size_t);
        }
    }
}
...
impl From<Advice> for RAdvice {
    fn from(value: Advice) -> Self {
        match value {
            ...
            Advice::DontNeed => Self::LinuxDontNeed,
            Advice::Free => Self::LinuxFree,
            Advice::Remove => Self::LinuxRemove,
            Advice::DontFork => Self::LinuxDontFork,
            Advice::DoFork => Self::LinuxDoFork,
            Advice::Mergeable => Self::LinuxMergeable,
            Advice::Unmergeable => Self::LinuxUnmergeable,
            Advice::HugePage => Self::LinuxHugepage,
            Advice::NoHugePage => Self::LinuxNoHugepage,
            Advice::DontDump => Self::LinuxDontDump,
            Advice::DoDump => Self::LinuxDoDump,
            Advice::HwPoison => Self::LinuxHwPoison,
        }
    }
}
